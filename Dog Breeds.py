...
def dog_size():
    ...
    if size == "tiny":
        ...
        for i in range(len(dogWeights)):
            if dogWeights[i] <= 25 and dogWeights[i] > 10:
                filteredlist.append(dogBreeds[i])
    elif size == "medium":
        for i in range(len(dogWeights)):
            if dogWeights[i] <= 60 and dogWeights[i] > 25:
                filteredlist.append(dogBreeds[i])
    elif size == "large":
        for i in range(len(dogWeights)):
            if dogWeights[i] <= 200 and dogWeights[i] > 60:
                filteredlist.append(dogBreeds[i])
    else:
        print("Enter valid choice")
        print("You will be given a random dog from the entire list.")
        filteredlist.append(dogBreeds[random.randint(0,(len(dogBreeds)-1))])
    print("Your recommended dog is...")
    time.sleep(3)
    randomdog = filteredlist[random.randint(0,(len(filteredlist)-1))]
...
def dog_breed():
    breedchoice = input("What breed group are you looking for?")
    breedchoice = breedchoice.lower()
    breedchoice = breedchoice.replace(" ","")
    index = 0
    for dogs in dogBreeds2:
        if dogs == breedchoice:
            index = dogBreeds2.index(breedchoice)
    traits = temperament[index]
    open_image(images[index])
    print(breedchoice + " traits:")
    print(traits)

def bred_For():
    forchoice = input("What is you desired purpose for the dog? (Ex: fish, circus, hunt)")
    forchoice = forchoice.lower()
    for i in range(len(bredFor)):
        if forchoice in bredFor[i].lower():
            filteredlist2.append(dogBreeds[i])
    if filteredlist2 == []:
        print("You entered invalid choice and don't get recommendation.")
    else:
        print("Your recommended dog is...")
        time.sleep(3)
        randomDog2 = filteredlist2[random.randint(0,(len(filteredlist2)-1))]
        recommendedDogs.append(randomDog2)
        print(randomDog2)

def menu(amountofdogs):
    while True:
        print("""Select what you would like to do:
        1. Random dog based off size preference
        2. Find image and traits of desired dog
        3. Random dog based off specified purpose
        4. End your dog finding journey""")
        try:
            menuchoice = int(input("Select choice 1-4:"))
        except:
        ...
        if menuchoice == 1:
            ...
            print("These were all you recommendations:")
...
#Main
print("Welcome to find your Dog!")
print("This simulator takes your preferences and finds your the perfect dog.")
menu(2)
